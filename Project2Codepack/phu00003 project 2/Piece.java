...
public class Piece {
    ...
    public void setPosition(int row, int col) {
        ...
        String choice;
        char newCharacter = 'E'; //E by default to initialize newCharacter and prevent program from crashing but still indicate error

        //Check for pawn promotion
        if (this.character == '\u2659' && row == 0) {
            //begin prompt for white pawn promotion

            System.out.println("Your pawn is elligble for promotion, type R, K, Q, B to promote (Rook, Knight, Queen, Bishop)");
            choice = scanner.next();
            switch (choice) {
                case "R":
                    newCharacter = '\u2656';
        ...
        } else if (character == '\u265F' && row == 7) {
            //begin prompt for black pawn promotion
